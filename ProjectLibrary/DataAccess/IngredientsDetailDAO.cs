...
namespace ProjectLibrary.DataAccess
{
	public class IngredientsDetailDAO
	{
		...
		public List<IngredientsDetail> GetIngredientsDetailByRecipeId(int recipeId)
		{
			var list = new List<IngredientsDetail>();
			try
			{
				using (var context = new CookingWebsiteContext())
				{
					list = context.IngredientsDetails
		...
		public void SaveIngredientsDetail(IngredientsDetail ingredientsDetail)
		{
			try
			{
				using (var context = new CookingWebsiteContext())
				{
					var existingIngredient = context.IngredientsDetails.FirstOrDefault(x => x.IngredientId == ingredientsDetail.IngredientId && x.Stt == ingredientsDetail.Stt && x.RecipeId == ingredientsDetail.RecipeId);
